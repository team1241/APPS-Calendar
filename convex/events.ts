...
function utcDay(offset: number, hour = 0): number {
  const today = new Date();
  return Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate() + offset, hour);
}
...
export const seedMockData = mutation({
  ...
  handler: async (ctx) => {
    ...
    const eventTypeName = "standard events";
    const existingEventType = await ctx.db
      .query("eventTypes")
      .withIndex("by_name", (q) => q.eq("name", eventTypeName))
      .unique();
    const eventTypeId = existingEventType?._id ?? (await ctx.db.insert("eventTypes", {
      name: eventTypeName,
      updatedAt: Date.now(),
    }));
    ...
    for (const subteamName of subteamNames) {
      const existingSubteam = await ctx.db
        .query("subteams")
        .withIndex("by_subteamName", (q) => q.eq("subteamName", subteamName))
        .unique();
      const subteamId = existingSubteam?._id ?? (await ctx.db.insert("subteams", {
        subteamName,
        updatedAt: Date.now(),
      }));
      subteamIds.set(subteamName, subteamId);
    }

    const programming = subteamIds.get("Programming");
    const manufacturing = subteamIds.get("Manufacturing");
    const electrical = subteamIds.get("Electrical");
    const design = subteamIds.get("Design");
    if (!programming || !manufacturing || !electrical || !design) {
      throw new Error("Unable to prepare mock subteams");
    }
    ...
    for (const mockEvent of mockEvents) {
      const existing = await ctx.db
        .query("events")
        .withIndex("by_title", (q) => q.eq("title", mockEvent.title))
        .unique();
      if (existing) continue;
      await ctx.db.insert("events", {
        title: mockEvent.title,
        startTime: mockEvent.start,
        endTime: mockEvent.end,
        allDay: mockEvent.allDay ?? false,
        eventTypeId,
        subteams: mockEvent.subteams,
        organizer: "APPS Calendar Demo",
        author: user._id,
